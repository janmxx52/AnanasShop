    ...
    <section className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold text-slate-900">Quản lý biến thể và ảnh sản phẩm</h1>
          <p className="text-sm text-slate-600">Quản trị biến thể tồn kho và thư viện ảnh cho sản phẩm #{product.id}.</p>
        </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="text-base font-semibold text-slate-900">Thông tin sản phẩm</h2>
        <div className="mt-3 grid gap-2 text-sm text-slate-700 sm:grid-cols-2 lg:grid-cols-3">
          <p>
            <span className="font-medium text-slate-900">Tên:</span> {product.name}
          </p>
          <p>
            <span className="font-medium text-slate-900">Slug:</span> {product.slug}
          </p>
          <p>
            <span className="font-medium text-slate-900">Trạng thái:</span> {productStatusLabel}
          </p>
          <p>
            <span className="font-medium text-slate-900">Danh mục:</span> {product.category?.name || '-'}
          </p>
          <p>
            <span className="font-medium text-slate-900">Thương hiệu:</span> {product.brand?.name || '-'}
          </p>
          <p>
            <span className="font-medium text-slate-900">Giá hiển thị:</span>{' '}
      ...
      <section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="text-base font-semibold text-slate-900">Quản lý biến thể</h2>

        <form className="grid gap-3 md:grid-cols-2 lg:grid-cols-3" onSubmit={handleVariantSubmit}>
          <Input
            label="Kích thước"
        ...
        {isLoadingVariants ? (
          ...
          <div className="space-y-3">
            <div className="overflow-x-auto rounded border border-slate-200">
              <table className="min-w-[980px] bg-white text-sm">
                ...
                <tbody>
                  {variants.map((variant) => (
                    <tr key={variant.id} className="border-t border-slate-100">
                      <td className="px-3 py-2 text-slate-700">{variant.id}</td>
                      <td className="px-3 py-2 font-medium text-slate-900">{variant.size}</td>
                      <td className="px-3 py-2 text-slate-700">{variant.color}</td>
                      <td className="px-3 py-2 text-slate-700">{variant.color_hex || '-'}</td>
                      <td className="px-3 py-2 text-slate-700">{variant.sku || '-'}</td>
                      <td className="px-3 py-2 text-slate-700">{variant.stock}</td>
                      <td className="px-3 py-2 text-slate-700">{formatPriceAdjustment(variant.price_adjustment)}</td>
      ...
      <section className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="text-base font-semibold text-slate-900">Quản lý ảnh sản phẩm</h2>

        <form className="grid gap-3 md:grid-cols-2" onSubmit={handleUploadImage}>
          <label className="block space-y-1 md:col-span-2">
            <span className="block text-sm font-medium text-slate-700">Ảnh sản phẩm</span>
            <input
              type="file"
              accept="image/png,image/jpeg,image/jpg,image/webp"
              className="w-full rounded border border-slate-300 px-3 py-2 text-sm text-slate-900 outline-none transition file:mr-3 file:rounded file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-sm file:text-slate-700 hover:file:bg-slate-200"
              ...
              <article key={image.id} className="overflow-hidden rounded-lg border border-slate-200 bg-white">
                ...
                <div className="space-y-3 p-3">
                  <div className="space-y-1 text-xs text-slate-600">
                    <p>
                      <span className="font-medium text-slate-900">ID:</span> {image.id}
                    </p>
                    <p>
                      <span className="font-medium text-slate-900">Thứ tự:</span> {image.sort_order}
                    </p>
                    <p>
