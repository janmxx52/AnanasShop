    ...
    <article className="space-y-3 rounded-lg border border-slate-200 bg-white p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="space-y-1">
          <p className="text-sm font-medium text-slate-900">{review.user?.name ?? 'Anonymous user'}</p>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {review.images.map((image) => (
            <div key={image.id} className="aspect-square overflow-hidden rounded bg-slate-100">
              <img src={image.image_url} alt={`Review ${review.id} image ${image.id}`} className="h-full w-full object-cover" />
            </div>
          ))}
        </div>
      ) : null}
